...
func IsFollowed(userId int, toUserId int) bool {
	...
	// 如果记录已存在，返回true
	if result := db.Where("user_id = ? and to_user_id = ?", userId, toUserId).Find(&Relation{}); result.Error == nil && result.RowsAffected > 0 {
		return true
	}
	return false
}
...
func UnFollowAction(userId int, toUserId int) error {
	...
	err := db.Transaction(func(tx *gorm.DB) error {
		// 1.在relation表中删除一条记录
		if err := db.Where("user_id = ? and to_user_id = ?", userId, toUserId).Delete(Relation{}).Error; err != nil {
			return err
		}
		// 2.用户toUser粉丝数-1
		toUser := &User{Id: toUserId}
...
func DeleteFromRelation(userId int, toUserId int) error {
	db := config.GetDefaultDb()
	result := db.Where("user_id = ? and to_user_id = ?", userId, toUserId).Delete(Relation{})
	return result.Error
}
...
func FollowList(userId int) []*json_model.User {
	...
	db := config.GetDefaultDb()
	db.Model(&Relation{}).Select("to_user_id id, username, follow_count, follower_count").
...
func FollowerList(userId int) []*json_model.User {
	fl := make([]*json_model.User, 0)
	db := config.GetDefaultDb()
	db.Model(&Relation{}).Select("user_id id, username, follow_count, follower_count").
